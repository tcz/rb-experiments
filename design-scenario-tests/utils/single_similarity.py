...
def take_screenshot_and_find_large_svg(url, path, viewport_width, viewport_height):
    with sync_playwright() as p:
        ...
        has_large_svg = page.evaluate('''() => {
    function svgExceedsViewport(threshold) {
      ...
      for (const svg of document.querySelectorAll('svg')) {
        ...
        if (svgArea / viewportArea > threshold) {
          return true;
        }
      }
      return false;
    }        
    return svgExceedsViewport(0.5);
}''')

        browser.close()
        return has_large_svg
...
def calculate_metrics(predicted_markup, expected_screenshot_path, viewport_width, viewport_height):
...
if __name__ == '__main__':
    ...
    has_large_svg = take_screenshot_and_find_large_svg(args.predicted_url, args.predicted_screenshot_path, args.viewport_width, args.viewport_height)

    metrics = metrics(args.predicted_screenshot_path, args.expected_screenshot_path)
    metrics['has_large_svg'] = has_large_svg
